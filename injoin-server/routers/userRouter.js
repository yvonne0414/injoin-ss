...
router.get('/bartd/:user_id', async (req, res, next) => {
  ...
  [data3] = await pool.execute(
    'SELECT user_bartd_like.user_id,user_bartd_like.bartd_id AS id,user_list.name as user_name, bartd_list.name as name ,bartd_list.img as img FROM `user_bartd_like` JOIN user_list ON user_bartd_like.user_id = user_list.id JOIN bartd_list ON user_bartd_like.bartd_id = bartd_list.id WHERE user_bartd_like.user_id = ? ORDER BY user_bartd_like.id DESC LIMIT ? OFFSET ?',
